...
async def confirm_purchase(req: ConfirmRequest):
    ...
    # Extract sender from payment verification
    verified_payer = verification.get("sender", "")
    if verified_payer:
        await db.update_purchase(req.purchase_id, {"payer": verified_payer})
    ...
    try:
        ...
        # Use verified payer as owner, fallback to purchase.payer
        owner_wallet = verified_payer or purchase.get("payer", "")
